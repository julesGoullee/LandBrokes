...
const register = program => program.command(signature, { noHelp: true }).description(description).usage('--network <network> [options]').option('--skip-compile', 'skips contract compilation').option('-d, --deploy-stdlib', 'deploys a copy of the stdlib for development').option('--reset', 'redeploys all contracts (not only the ones that changed)').withNetworkOptions().action(action);

async function action(options) {
  const { skipCompile, deployStdlib, reset: reupload } = options;
  await (0, _runWithTruffle2.default)(async opts => await (0, _push2.default)(_extends({ deployStdlib, reupload }, opts)), _extends({ compile: !skipCompile }, options));
}
