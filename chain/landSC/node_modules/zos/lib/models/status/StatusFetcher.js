...
class StatusFetcher {
  ...
  onMissingRemoteContract(expected, observed, { alias, address }) {
    ...
    if (_zosLib.FileSystem.exists(buildPath)) {
      ...
      const bodyBytecodeHash = (0, _contracts.bytecodeDigest)(remoteBodyBytecode);
      if ((0, _contracts.bodyCode)(contract) === remoteBodyBytecode) {
        log.warn(`Assuming that constructor function of local version of ${contractName} is the one registered`);
        const constructor = (0, _contracts.constructorCode)(contract);
        const bytecodeHash = (0, _contracts.bytecodeDigest)(constructor + remoteBodyBytecode);
