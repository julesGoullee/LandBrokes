...
class NetworkAppController extends _NetworkBaseController2.default {
  ...
  async deploy() {
    this.app = await _zosLib.App.deploy(this.packageFile.version, this.txParams);
  ...
  async fetch() {
    ...
    this.app = await _zosLib.App.fetch(this.appAddress, this.txParams);
  }
  ...
  async deployStdlib() {
    ...
    const stdlibAddress = await _Stdlib2.default.deploy(this.packageFile.stdlibName, this.txParams);
    this.networkFile.stdlib = _extends({ address: stdlibAddress, customDeploy: true }, this.packageFile.stdlib);
  }
  ...
  getProxies(contractAlias, proxyAddress) {
    ...
    return {
      [contractAlias]: _lodash2.default.filter(this.networkFile.proxiesOf(contractAlias), proxy => !proxyAddress || proxy.address === proxyAddress)
    };
  }
  ...
  async linkStdlib() {
    ...
    const { address: stdlibAddress, version: stdlibVersion } = _Stdlib2.default.fetch(stdlibName, this.packageFile.stdlibVersion, this.networkFile.network);
    ...
    if (stdlibAddress !== currentStdlibAddress) {
      ...
      this.networkFile.stdlib = _extends({}, this.packageFile.stdlib, { // name, customDeploy
  ...
  isStdlibContract(contractAlias) {
    ...
    const stdlib = new _Stdlib2.default(this.packageFile.stdlibName);
